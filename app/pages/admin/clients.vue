  ...
  <NuxtLayout name="admin">
    <div class="space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 class="text-2xl font-semibold text-ink">OIDC Clients</h1>
          <p class="text-sm text-steel mt-1">Manage OAuth/OIDC client applications</p>
        </div>
        <UButton
      ...
      <div v-else-if="clients.length === 0" class="bg-canvas rounded-xl border border-hairline p-12 text-center">
        ...
        <h3 class="text-lg font-medium text-ink mb-2">No clients yet</h3>
        <p class="text-steel mb-4">Create your first OIDC client to get started</p>
        <UButton
          @click="showCreateModal = true"
          color="primary"
        >
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        >
          ...
          <!-- Card Body -->
          <div class="px-6 py-4 border-t border-hairline-soft flex-1 space-y-3">
            <div>
              <p class="text-xs font-medium text-steel uppercase tracking-wider mb-1">Client ID</p>
              <div class="flex items-center gap-2 group/id">
                <code class="text-sm bg-surface px-2 py-1 rounded border border-hairline-soft text-charcoal truncate font-mono">
                  {{ client.clientId }}
                </code>
                <UButton
            ...
            <div>
              <p class="text-xs font-medium text-steel uppercase tracking-wider mb-1">Redirect URIs</p>
              <div class="text-sm text-steel">
                ...
                <span v-else class="text-stone italic">No redirect URIs configured</span>
              </div>
            </div>
            ...
            <div class="pt-2 border-t border-hairline-soft">
              <div class="flex items-center justify-between">
                <div>
                  <p class="text-xs font-medium text-charcoal">Access Control</p>
      ...
      <div v-if="showCreateModal || showEditModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative bg-canvas rounded-2xl shadow-mm-2 w-full max-w-lg max-h-[90vh] overflow-y-auto">
          <div class="px-6 py-4 border-b border-hairline">
            <h3 class="text-lg font-semibold text-ink">{{ showEditModal ? 'Edit Client' : 'Create Client' }}</h3>
          </div>
          <form @submit.prevent="showEditModal ? updateClient() : createClient()" class="p-6 space-y-4">
            <div>
              <label class="block text-sm font-medium text-charcoal mb-2">Client Name</label>
              <UInput
            ...
            <div>
              <label class="block text-sm font-medium text-charcoal mb-2">Redirect URIs (one per line)</label>
              <UTextarea
      ...
      <div v-if="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative bg-canvas rounded-2xl shadow-mm-2 w-full max-w-md p-6">
          <div class="text-center">
            <div class="w-12 h-12 mx-auto mb-4 rounded-full bg-[#d45656]/10 flex items-center justify-center">
              <svg class="w-6 h-6 text-[#d45656]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
              </svg>
            </div>
            <h3 class="text-lg font-semibold text-ink mb-2">Delete Client</h3>
            <p class="text-steel mb-6">Are you sure you want to delete "{{ clientToDelete?.clientName }}"? This action cannot be undone.</p>
      ...
      <div v-if="showSecretModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative bg-canvas rounded-2xl shadow-mm-2 w-full max-w-lg p-6">
          <div class="text-center mb-6">
            <div class="w-12 h-12 mx-auto mb-4 rounded-full bg-success-bg flex items-center justify-center">
              <svg class="w-6 h-6 text-success-text" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
              </svg>
            </div>
            <h3 class="text-lg font-semibold text-ink mb-2">Client Created Successfully!</h3>
            <p class="text-steel text-sm font-medium">Save these credentials now — the secret will not be shown again.</p>
          </div>
          
          <div class="space-y-4">
            <div>
              <label class="block text-sm font-medium text-charcoal mb-1">Client ID</label>
              <div class="flex items-center gap-2">
                <input
                  type="text"
                  readonly
                  :value="createdClient?.clientId"
                  class="flex-1 px-4 py-2 bg-surface border border-hairline rounded-lg text-sm font-mono"
                />
                <button
                  @click="copyToClipboard(createdClient?.clientId)"
                  class="px-3 py-2 bg-surface hover:bg-hairline rounded-lg transition-colors"
                  title="Copy"
                >
            ...
            <div v-if="createdClient?.clientSecret">
              <label class="block text-sm font-medium text-charcoal mb-1">Client Secret</label>
              <div class="flex items-center gap-2">
                <input
      ...
      >
        <div v-if="toast.show" class="fixed bottom-4 right-4 z-50">
          <div :class="[
            'flex items-center gap-3 px-4 py-3 rounded-lg shadow-mm-2 min-w-[280px]',
            toast.type === 'success' ? 'bg-success-text text-white' : '',
            toast.type === 'error' ? 'bg-[#d45656] text-white' : '',
            ...
            <button @click="toast.show = false" class="ml-auto p-1 hover:bg-canvas/20 rounded transition-colors">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
...
async function createClient() {
  ...
  try {
    ...
    showToast("Failed to create client: " + (error?.data?.message || "Unknown error"), "error");
  } finally {
    saving.value = false;
  }
}
...
async function toggleAccessControl(client) {
  ...
  try {
    ...
    const message = client.requireAccessGrant ? "Access control enabled - only granted users can access this app" : "Access control disabled - all users can access this app";
