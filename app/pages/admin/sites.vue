<template>
  <div>
        ...
        <div class="px-6 py-4 bg-surface border-b border-hairline">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
              ...
              <div>
                <h3 class="font-semibold text-lg text-ink">{{ site.name }}</h3>
                <p class="text-steel text-sm">{{ site.code }}</p>
              </div>
            </div>
            <div class="flex items-center gap-2">
              <span 
                class="px-2.5 py-1 text-xs font-medium rounded-full"
                :class="site.isActive ? 'bg-success-bg text-success-text' : 'bg-[#d45656]/10 text-[#d45656]'"
              >
                {{ site.isActive ? 'Aktif' : 'Nonaktif' }}
              </span>
              <button
                @click="toggleTreeExpand(site.id)"
                class="p-1.5 text-steel hover:text-ink hover:bg-hairline-soft rounded-md transition-colors"
          ...
          <p v-if="site.address" class="text-steel text-sm mt-2">{{ site.address }}</p>
        </div>

        <!-- Site Tree (Expanded) -->
        <div v-if="expandedSites.has(site.id)" class="border-t border-hairline-soft">
          <!-- Loading State -->
          <div v-if="loadingTrees.has(site.id)" class="p-6 text-center text-steel">
          ...
          <div v-else-if="siteTrees[site.id]" class="p-4">
            ...
            <div v-if="siteTrees[site.id].tree.length > 0" class="space-y-2">
              <div 
                ...
                :key="division.id"
                class="border border-hairline rounded-lg overflow-hidden"
              >
                <!-- Division Header -->
                <div 
                  class="flex items-center gap-3 px-4 py-3 bg-surface cursor-pointer hover:bg-surface transition-colors"
                ...
                >
                  ...
                  <div class="flex-1">
                    <p class="font-medium text-ink">{{ division.name }}</p>
                    <p class="text-xs text-steel">{{ division.code }} · {{ division.children.length }} unit</p>
                  </div>
                  <span 
                    class="px-2 py-0.5 text-xs rounded-full"
                    :class="division.isActive ? 'bg-success-bg text-success-text' : 'bg-surface text-steel'"
                  >
                ...
                <div 
                  ...
                  class="border-t border-hairline-soft bg-canvas"
                >
                  <div 
                    v-for="(unit, index) in division.children" 
                    :key="unit.id"
                    class="flex items-center gap-3 px-4 py-2 pl-12 hover:bg-surface"
                    :class="{ 'border-t border-hairline-soft': index > 0 }"
                  >
                    <div class="w-6 h-6 bg-surface rounded flex items-center justify-center">
                    ...
                    <div class="flex-1">
                      <p class="text-sm text-ink">{{ unit.name }}</p>
                      <p class="text-xs text-steel">{{ unit.code }}</p>
                    </div>
                    <span 
                      class="px-2 py-0.5 text-xs rounded-full"
                      :class="unit.isActive ? 'bg-success-bg text-success-text' : 'bg-surface text-steel'"
                    >
                ...
                <div 
                  v-if="expandedDivisions.has(`${site.id}-${division.id}`) && division.children.length === 0"
                  class="border-t border-hairline-soft px-4 py-3 pl-12 text-sm text-stone italic"
                >
    ...
    <div v-if="sites.length === 0" class="bg-canvas shadow-mm-1 rounded-lg p-12 text-center">
      ...
      <h3 class="text-lg font-medium text-ink mb-2">Belum Ada Site</h3>
      <p class="text-steel mb-4">Mulai dengan menambahkan site pertama Anda</p>
      <UButton
        @click="openCreateModal"
        color="primary"
        icon="i-lucide-plus"
    ...
    <Teleport to="body">
      <div v-if="showModal" class="fixed inset-0 z-50 overflow-y-auto">
        ...
        <div class="flex min-h-full items-center justify-center p-4">
          <div class="relative bg-canvas rounded-lg shadow-mm-2 max-w-2xl w-full">
            <div class="px-6 pt-6 pb-4 border-b border-hairline">
            ...
            <form @submit.prevent="submitForm" class="p-6 space-y-4">
              <div>
                <label class="block text-sm font-medium text-charcoal mb-1">Kode Site</label>
                <UInput
              ...
              <div>
                <label class="block text-sm font-medium text-charcoal mb-1">Nama Site</label>
                <UInput
              ...
              <div>
                <label class="block text-sm font-medium text-charcoal mb-1">Alamat</label>
                <UTextarea
              ...
              <div class="pt-4 border-t border-hairline">
                <h4 class="text-sm font-medium text-ink mb-3">Pengaturan Struktur Organisasi</h4>
                <div class="space-y-3">
                  <div class="flex items-center justify-between">
                    <div>
                      <label class="text-sm font-medium text-charcoal">Gunakan Divisi</label>
                      <p class="text-xs text-steel">Aktifkan untuk menggunakan struktur divisi</p>
                    </div>
                    <button
                      type="button"
                      @click="toggleDivisions"
                      :class="[
                        'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-brand-blue-deep focus:ring-offset-2',
                        formData.useDivisions ? 'bg-primary' : 'bg-hairline'
                      ]"
                    >
                      <span
                        :class="[
                  ...
                  <div class="flex items-center justify-between" :class="{ 'opacity-50': !formData.useDivisions }">
                    <div>
                      <label class="text-sm font-medium text-charcoal">Gunakan Unit</label>
                      <p class="text-xs text-steel">Aktifkan untuk menggunakan struktur unit (membutuhkan divisi)</p>
                    </div>
                    <button
                      type="button"
                      @click="toggleUnits"
                      ...
                      :class="[
                        'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-brand-blue-deep focus:ring-offset-2',
                        formData.useUnits ? 'bg-primary' : 'bg-hairline',
              ...
              <div class="pt-4 flex justify-end gap-3 border-t border-hairline">
                <UButton
                  type="button"
                  @click="closeModal"
                  variant="outline"
...
const expandedDivisions = ref(/* @__PURE__ */ new Set());
const loadingTrees = ref(/* @__PURE__ */ new Set());
const siteTrees = ref({});
const formData = ref({
  code: "",
  name: "",
...
async function toggleTreeExpand(siteId) {
...
function toggleDivisionExpand(siteId, divisionId) {
...
async function loadSiteTree(siteId) {
  loadingTrees.value.add(siteId);
  loadingTrees.value = new Set(loadingTrees.value);
  try {
    const response = await $fetch(`/api/admin/sites/${siteId}/tree`);
