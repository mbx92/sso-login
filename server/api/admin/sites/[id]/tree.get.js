...
var tree_get_default = defineEventHandler(async (event) => {
  ...
  try {
    ...
    }).from(divisions).where(eq(divisions.siteId, siteId)).orderBy(divisions.name);
    ...
    }).from(units).where(eq(units.siteId, siteId)).orderBy(units.name);
    const tree = siteDivisions.map((division) => ({
      ...division,
      type: "division",
      children: siteUnits.filter((unit) => unit.divisionId === division.id).map((unit) => ({
        ...unit,
        type: "unit"
      }))
    }));
