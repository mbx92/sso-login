...
var id_delete_default = defineEventHandler(async (event) => {
  ...
  try {
    ...
    }).from(userAppAccess).leftJoin(users, eq(userAppAccess.userId, users.id)).leftJoin(oidcClients, eq(userAppAccess.clientId, oidcClients.id)).where(eq(userAppAccess.id, accessId)).limit(1);
    if (!existingAccess) {
      throw createError({
        statusCode: 404,
    ...
    await db.delete(userAppAccess).where(eq(userAppAccess.id, accessId));
    const sessionUser = getSessionUser(event);
    await writeAuditLog({
      action: "ADMIN_USER_ACCESS_REVOKED",
