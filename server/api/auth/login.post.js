...
var login_post_default = defineEventHandler(async (event) => {
  try {
    ...
    }).from(users).leftJoin(units, eq(users.unitId, units.id)).where(eq(users.email, email.toLowerCase())).limit(1);
    ...
    if (!siteId) {
      const userRoleResult = await db.select({ siteId: roles.siteId }).from(userRoles).innerJoin(roles, eq(userRoles.roleId, roles.id)).where(eq(userRoles.userId, user.id)).limit(1);
    ...
    throw createError({
      statusCode: 500,
      statusMessage: `An error occurred during login: ${error.message || "Unknown error"}`
    });
  }
});
export {
  login_post_default as default
};
