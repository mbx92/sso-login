...
const divisions = pgTable("divisions", {
  id: uuid("id").primaryKey().defaultRandom(),
...
const roles = pgTable("roles", {
  ...
  description: text("description"),
  permissions: jsonb("permissions").notNull().$type().default([]),
...
const oidcClients = pgTable("oidc_clients", {
  id: uuid("id").primaryKey().defaultRandom(),
  ...
  // null = global client
  redirectUris: jsonb("redirect_uris").notNull().$type().default([]),
  postLogoutRedirectUris: jsonb("post_logout_redirect_uris").$type().default([]),
  grantTypes: jsonb("grant_types").notNull().$type().default(["authorization_code"]),
  responseTypes: jsonb("response_types").notNull().$type().default(["code"]),
  scopes: jsonb("scopes").notNull().$type().default(["openid", "profile", "email"]),
...
const userAppAccess = pgTable("user_app_access", {
  id: uuid("id").primaryKey().defaultRandom(),
  userId: uuid("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
...
const accessGroups = pgTable("access_groups", {
  id: uuid("id").primaryKey().defaultRandom(),
  name: varchar("name", { length: 255 }).notNull(),
  description: text("description"),
  siteId: uuid("site_id").references(() => sites.id, { onDelete: "set null" }),
...
const accessGroupUsers = pgTable("access_group_users", {
  id: uuid("id").primaryKey().defaultRandom(),
...
const accessGroupClients = pgTable("access_group_clients", {
  id: uuid("id").primaryKey().defaultRandom(),
  groupId: uuid("group_id").notNull().references(() => accessGroups.id, { onDelete: "cascade" }),
  clientId: uuid("client_id").notNull().references(() => oidcClients.id, { onDelete: "cascade" }),
  addedBy: uuid("added_by").references(() => users.id, { onDelete: "set null" }),
...
const oidcKv = pgTable("oidc_kv", {
  id: uuid("id").primaryKey().defaultRandom(),
  model: varchar("model", { length: 50 }).notNull(),
  // Grant, Session, AccessToken, etc.
...
const divisionsRelations = relations(divisions, ({ one, many }) => ({
  site: one(sites, {
...
const unitsRelations = relations(units, ({ one, many }) => ({
  site: one(sites, {
    fields: [units.siteId],
    references: [sites.id]
  }),
...
const oidcClientsRelations = relations(oidcClients, ({ many }) => ({
  userAccess: many(userAppAccess),
  groupAccess: many(accessGroupClients)
}));
const userAppAccessRelations = relations(userAppAccess, ({ one }) => ({
  user: one(users, {
...
const accessGroupsRelations = relations(accessGroups, ({ one, many }) => ({
  site: one(sites, {
...
const accessGroupUsersRelations = relations(accessGroupUsers, ({ one }) => ({
...
const accessGroupClientsRelations = relations(accessGroupClients, ({ one }) => ({
  group: one(accessGroups, {
